...
object Main {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val queries: List[String] = DATA_SET match {
            ...
            case "company" => queryWorkloadCompany()
        }
...
//            println(l.split(";")(3)) // can cause array out of bounds exception for some queries
    ...
    /**
     * Loads the query log from the company files into a list of usable SQL queries
     *
     * @return a List of query SQL statements
     */
    def queryWorkloadCompany(): List[String] = {
        return List()
    }
    ...
    def processQueries(queries: List[String]): Seq[Set[LogicalPlan]] = {
        ...
        for (i <- queries.indices) {
            ...
            try {
                ...
                println(logicalPlan)
                val subTrees: Set[LogicalPlan] = getPartialQEPs(logicalPlan)
